...
class UtilModel(val context: Context): IUtilModel {
    ...
    override fun saveToken(response: Response<LoginResponse>) {
        PrefManager.putString(Constant.ACCESS_TOKEN, response.body().access_token as String)
        val validity = System.currentTimeMillis() + response.body().valid_seconds * 1000
