...
then
	...
	if [ "$CIRCLE_BRANCH" == "$PUBLISH_BRANCH" ]; then
		...
		jarsigner -verbose -tsa http://timestamp.comodoca.com/rfc3161 -sigalg SHA1withRSA -digestalg SHA1 -keystore ../exec/key.jks -storepass $STORE_PASS -keypass $KEY_PASS app-fdroid-release-unaligned.apk $ALIAS
	...
	curl https://$APPETIZE_API_TOKEN@api.appetize.io/v1/apps/mbpprq4xj92c119j7nxdhttjm0 -H 'Content-Type: application/json' -d '{"url":"https://github.com/fossasia/susi_android/raw/apk/app-fdroid-debug.apk", "note": "Update SUSI Preview"}'
	...
	if [ "$CIRCLE_BRANCH" == "$PUBLISH_BRANCH" ]; then
		...
		fastlane supply --apk app-fdroid-release.apk --track alpha --json_key ../exec/fastlane.json --package_name $PACKAGE_NAME
