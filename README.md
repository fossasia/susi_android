...
## Naming Conventions
Organizing file systems has a big impact on your ability to find the file later on and hence a proper naming convention is necessary while naming files, variables, ID's etc. You should be consistent and descriptive in naming and organizing files so that it is obvious where to find specific data and what the files contain.
...
3. File name should depict its purpose.
4. All the class files should begin with upper camelcase. For example: signup activity should be written as SignUpActivity.
5. The value files should always be in the plural. Examples: strings.xml, colors.xml,dimens.xml.
6. Drawable files:
